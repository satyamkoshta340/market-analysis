...
for k in range(0,len(stk)):
	...
	from_datetime = datetime.datetime(2023, 4, 10, 9, 00, 00, 000000)
	to_datetime = datetime.datetime(2023, 4, 10, 11, 00, 00, 000000)
	interval = "15minute"
			...
			if dt["high"][i] <150 or dt["high"][i] > 7999:
				...
				if first_15m_high > max(dt["high"][i+1:i+5]) and first_15m_low < min(dt["low"][i+1:i+5]):
					...
					if min(dt["low"][i+1:i+5]) >= (first_15m_high - rsfr):
						print ("Condition satisfied to place buy order", stk["EQ"][k], dt["close"][i+5], dt["volume"][i+5],dt["date"][i+5])
						if pct_candle <= 3.5:
							filtered_scan1.append((pct_candle, stk["EQ"][k], first_15m_high))
					if max(dt["high"][i+1:i+5]) <= (first_15m_low + rsfr):
						print ("Condition satisfied to place sell order", stk["EQ"][k], dt["close"][i+5], dt["volume"][i+5],dt["date"][i+5])
						if pct_candle <= 3.5:
...
if test_run == "No":
	...
	if len(filtered_scan1) >= 1:
		...
		print ("Placing 1st buying order for", tsb, "at", entry_price, qty)
		...
		try:
			...
			entry_price = round((filtered_scan1[stk_no][2] + filtered_scan1[stk_no][2]*0.0001),1)
			qty = int(5*iniCap/entry_price)
			print ("Placing 2nd buying order for", tsb, "at", entry_price, qty)
			order = kite.place_order(variety=kite.VARIETY_REGULAR,
			                         exchange=kite.EXCHANGE_NSE,
			                         tradingsymbol=tsb,
			                         transaction_type=kite.TRANSACTION_TYPE_BUY,
		...
		try:
			...
			tsb = filtered_scan1[stk_no][1]
			qty = int(5*iniCap/entry_price)
			print ("Placing 3rd buying order for", tsb, "at", entry_price, qty)
			order = kite.place_order(variety=kite.VARIETY_REGULAR,
			                         exchange=kite.EXCHANGE_NSE,
			                         tradingsymbol=tsb,
			                         transaction_type=kite.TRANSACTION_TYPE_BUY,
	...
	if len(filtered_scan2) >= 1 :
		...
		qty = int(5*iniCap/entry_price)
		print ("Placing 1st sell order for", tsb, "at", entry_price, qty)
		order = kite.place_order(variety=kite.VARIETY_REGULAR,
		                         exchange=kite.EXCHANGE_NSE,
		                         tradingsymbol=tsb,
		...
		try:
			...
			entry_price = round((filtered_scan2[stk_no][2] - filtered_scan2[stk_no][2]*0.0001),1)
			qty = int(5*iniCap/entry_price)
			print ("Placing 2nd sell order for", tsb, "at", entry_price, qty)
			order = kite.place_order(variety=kite.VARIETY_REGULAR,
			                         exchange=kite.EXCHANGE_NSE,
			                         transaction_type=kite.TRANSACTION_TYPE_SELL,
			                         quantity=qty,
